...
<!Doctype html>
	...
	<head>
		...
		$filename = "laporan_surat_keluar_" . date('YmdHis') . ".xls";
...
<body>
	...
	if ( isset($maxDate) ) {
	 ...
	 ?>
	<h2 class="title-text"><?= "Laporan Surat Keluar ".date('d F Y', strtotime($min)).' - '.date('d F Y', strtotime($max)) ?></h2>
		...
		if ( isset($check) and $check != 0 ) {
			...
			foreach ($check as $row) {
				...
				<tr align="center">
					...
					<td><?=$row->pengirim ?></td>
					<td><?=$row->jabatan_pengirim ?></td>
					<td><?=$row->penerima ?></td>
					<td><?=$row->jabatan_penerima ?></td>
					<td><?=$row->instansi ?></td>
